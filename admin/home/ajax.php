   ...
   if (isset($_POST["user_log_list"])){
      ...
      if($searchValue != ''){
         $searchQuery = " AND (log_id LIKE :log_id OR 
            ...
            DATE_FORMAT(log_date, '%M %d, %Y %H:%i:%s %p') LIKE :new_log_date OR) ";
         $searchArray = array( 
            'log_id'=>"%$searchValue%",
      ...
      $totalRecordwithFilter = $records['allcount'];
      // Fetch records
      $stmt = $conn->prepare("SELECT *, DATE_FORMAT(log_date, '%M %d, %Y %H:%i:%s %p') as new_log_date FROM user_logs INNER JOIN user ON user.user_id = user_logs.user_id WHERE user_logs.user_id = '$user_id' AND is_user_log = '1'".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
      // Bind values
      foreach ($searchArray as $key=>$search){
         $stmt->bindValue(':'.$key, $search,PDO::PARAM_STR);
      }
